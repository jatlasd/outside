...
export async function getWeather(flags, location = DEFAULT_LOCATION, options = {}) {
  ...
  const pastRaw = Number(options.pastDays)
  const pastDays = Number.isFinite(pastRaw) ? Math.min(92, Math.max(0, Math.trunc(pastRaw))) : 0
  const pastQuery = pastDays > 0 ? `&past_days=${pastDays}` : ""
  ...
  if (needsForecastFetch(flags) && forecastVars) {
    const response = await fetch(
      `https://api.open-meteo.com/v1/forecast?latitude=${latitude}&longitude=${longitude}&hourly=${forecastVars}&${tz}${pastQuery}`,
    )
  ...
  if (needsAirQualityFetch(flags) && airVars) {
    const response = await fetch(
      `https://air-quality-api.open-meteo.com/v1/air-quality?latitude=${latitude}&longitude=${longitude}&hourly=${airVars}&${tz}${pastQuery}`,
    )

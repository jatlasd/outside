...
export function normalizeHourly(forecast, air, flags) {
  ...
  const times =
    fc?.time?.length ? fc.time : aq?.time?.length ? aq.time : null
  ...
  return times.map((time) => {
    ...
    const getFc = (key) =>
      fi != null && fc?.[key] ? fc[key][fi] ?? null : null
    const getAq = (key) =>
      ai != null && aq?.[key] ? aq[key][ai] ?? null : null

    if (flags.temperature) row.temperatureC = getFc("temperature_2m")
    if (flags.apparentTemperature)
      row.apparentTempC = getFc("apparent_temperature")
    ...
    if (flags.precipitationProbability)
      row.precipProbabilityPct = getFc("precipitation_probability")
...
export function calendarDatePrefixInTimeZone(ianaTimeZone, dayOffset = 0, anchorDate = new Date()) {
  const tz = ianaTimeZone || "UTC"
  const delta = Math.trunc(dayOffset)
  let y
  let mo
  let d
  try {
    ...
    return todayDatePrefixInTimeZone(tz)
  }
  if (!Number.isFinite(y) || !Number.isFinite(mo) || !Number.isFinite(d)) {
    return todayDatePrefixInTimeZone(tz)
  }
  const rolled = new Date(Date.UTC(y, mo - 1, d + delta))
  const ny = rolled.getUTCFullYear()
  const nmo = rolled.getUTCMonth() + 1
  const nd = rolled.getUTCDate()
  const mm = String(nmo).padStart(2, "0")
  const dd = String(nd).padStart(2, "0")
  return `${ny}-${mm}-${dd}`
}

export function sameClockHourOnDatePrefix(timeKey, datePrefix) {
  if (typeof timeKey !== "string" || typeof datePrefix !== "string") return null
  const t = timeKey.indexOf("T")
  if (t === -1) return null
  if (!/^\d{4}-\d{2}-\d{2}$/.test(datePrefix)) return null
  return `${datePrefix}${timeKey.slice(t)}`
}

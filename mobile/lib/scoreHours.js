import { PARAMS } from "./paramConfig";
...
function impactSummaryForBand(band) {
  ...
  if (band === "moderate") return "Outside conditions may add noticeable strain through parts of the day.";
  if (band === "high") return "Outside conditions are likely a major contributor for much of today.";
  if (band === "severe") return "Outside pressure is intense today and may dominate how you feel.";
...
function weatherCodePenalty(code) {
  ...
  if (c >= 95) return { score: 26, reason: "Thunderstorm conditions (weather code)" };
  if (c >= 71 && c <= 77) return { score: 12, reason: "Snow / ice pellets (weather code)" };
  if (c === 45 || c === 48) return { score: 10, reason: "Fog (weather code)" };
  if (c >= 82 || c === 65) return { score: 10, reason: "Heavy rain or intense showers (weather code)" };
  if (c === 63 || c === 81) return { score: 6, reason: "Moderate rain or showers (weather code)" };
  return { score: 0, reason: null };
}

const PARAM_LABEL_BY_ID = Object.fromEntries(PARAMS.map((p) => [p.id, p.label]));
...
export function rollupBreakdown(scored) {
  ...
  if (otherRaw > 0) {
    items.push({ id: "__other__", label: "Other", raw: otherRaw, pct: (100 * otherRaw) / totalRaw });
  }
...
export function topBreakdownContributors(breakdown, limit = 2) {
  ...
  const entries = Object.entries(breakdown).filter(([, raw]) => Number.isFinite(raw) && raw > 0).sort((a, b) => b[1] - a[1]);
  if (!entries.length) return [];
  const safeLimit = Math.max(1, Math.trunc(limit));
  const top = entries.slice(0, safeLimit);
  const total = entries.reduce((sum, [, raw]) => sum + raw, 0);
  return top.map(([id, raw]) => ({ id, raw, pct: total > 0 ? (100 * raw) / total : 0 }));
}
...
export function scoreHours(hours, flags, weights) {
  return hours.map((hour, i) => {
    ...
    if (flags.dewpoint && wt("dewpoint") > 0) {
      ...
      if (d != null) {
        if (d >= 24) { const add = 12 * w; score += add; bump(breakdown, "dewpoint", add); reasons.push("Very muggy dew point"); }
        else if (d >= 20) { const add = 7 * w; score += add; bump(breakdown, "dewpoint", add); reasons.push("Elevated dew point"); }
      }
    }
    ...
    if (flags.precipitation && wt("precipitation", "high") > 0) {
      ...
      if (rain != null) {
        ...
        else if (rain >= 0.3) { const add = 10 * w; score += add; bump(breakdown, "precipitation", add); reasons.push("Wet / changing conditions"); }
      }
    }
    ...
    if (flags.precipitationProbability && wt("precipitationProbability", "high") > 0) {
      ...
      if (p != null) {
        ...
        else if (p >= 70) { const add = 5 * w; score += add; bump(breakdown, "precipitationProbability", add); reasons.push("High precipitation probability"); }
      }
    }
    ...
    if (flags.pressure && wt("pressure") > 0 && i > 0) {
      ...
      if (prev != null && cur != null) {
        ...
        if (delta >= 5) { const add = 24 * w; score += add; bump(breakdown, "pressure", add); reasons.push("Strong hour-over-hour pressure change"); }
        else if (delta >= 3) { const add = 14 * w; score += add; bump(breakdown, "pressure", add); reasons.push("Notable pressure change"); }
      }
    }
    ...
    if (flags.windGusts && wt("windGusts") > 0 && i > 0) {
      ...
      if (prev != null && cur != null) {
        const delta = Math.abs(cur - prev);
        if (delta >= 20) { const add = 14 * w; score += add; bump(breakdown, "windGusts", add); reasons.push("Sharp increase in wind gusts"); }
        else if (delta >= 12) { const add = 8 * w; score += add; bump(breakdown, "windGusts", add); reasons.push("Gusts changing vs prior hour"); }
      }
    }
    ...
    if (flags.visibility && wt("visibility") > 0) {
      ...
      if (vis != null) {
        if (vis < 1000) { const add = 16 * w; score += add; bump(breakdown, "visibility", add); reasons.push("Very poor visibility"); }
        else if (vis < 4000) { const add = 9 * w; score += add; bump(breakdown, "visibility", add); reasons.push("Reduced visibility"); }
      }
    }
    ...
    if (flags.uv && wt("uv") > 0) {
      ...
      if (uv != null) {
        if (uv >= 6) { const add = 28 * w; score += add; bump(breakdown, "uv", add); reasons.push("High UV exposure risk"); }
        else if (uv >= 3) { const add = 12 * w; score += add; bump(breakdown, "uv", add); reasons.push("Moderate UV"); }
      }
    }
    ...
    if (flags.aqi && wt("aqi") > 0) {
      ...
      if (aqi != null) {
        if (aqi >= 75) { const add = 26 * w; score += add; bump(breakdown, "aqi", add); reasons.push("Poor European AQI"); }
        else if (aqi >= 55) { const add = 16 * w; score += add; bump(breakdown, "aqi", add); reasons.push("Moderate\u2013elevated AQI"); }
        else if (aqi >= 40) { const add = 8 * w; score += add; bump(breakdown, "aqi", add); reasons.push("Fair AQI"); }
      }
    }
    ...
    if (flags.particulates && wt("particulates") > 0) {
      ...
      let add = 0;
      if (pm25 != null) { if (pm25 >= 50) add += 22; else if (pm25 >= 25) add += 14; else if (pm25 >= 15) add += 7; }
      if (pm10 != null) { if (pm10 >= 100) add += 14; else if (pm10 >= 50) add += 9; }
      add *= w;
    ...
    if (flags.ozone && wt("ozone") > 0) {
      ...
      if (o != null) {
        let add = 0;
        if (o >= 180) add += 18; else if (o >= 120) add += 11; else if (o >= 100) add += 6;
        add *= w;
    ...
    if (flags.nitrogenDioxide && wt("nitrogenDioxide") > 0) {
      ...
      if (no2 != null) {
        let add = 0;
        if (no2 >= 200) add += 16; else if (no2 >= 120) add += 10; else if (no2 >= 80) add += 5;
        add *= w;
    ...
    if (flags.carbonMonoxide && wt("carbonMonoxide") > 0) {
      ...
      if (co != null) {
        let add = 0;
        if (co >= 5000) add += 14; else if (co >= 2000) add += 8;
        add *= w;
    ...
    if (flags.pollen && wt("pollen") > 0) {
      ...
      if (pollen != null) {
        if (pollen >= 80) { const add = 22 * w; score += add; bump(breakdown, "pollen", add); reasons.push("Very high pollen"); }
        else if (pollen >= 40) { const add = 14 * w; score += add; bump(breakdown, "pollen", add); reasons.push("High pollen"); }
        else if (pollen >= 15) { const add = 8 * w; score += add; bump(breakdown, "pollen", add); reasons.push("Moderate pollen"); }
      }
    }

    return { ...hour, score: clampScore(Math.round(score)), reasons, breakdown };
  });
}
...
export function rollupHours(scored) {
  ...
  const meanScoreRaw = scored.reduce((acc, row) => acc + row.score, 0) / scored.length;
  const meanScore = clampScore(Math.round(meanScoreRaw));
  const worstScore = clampScore(Math.round(worst.score ?? 0));
  const dayImpactScore = clampScore(Math.round(meanScore * 0.6 + worstScore * 0.4));
  ...
  return { worst, worstScore, meanScore, dayImpactScore, impactBand, impactSummary, hours: scored, breakdown };
}
